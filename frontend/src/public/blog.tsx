...
import { Heart, Sparkles, HandHeart, PartyPopper, Lightbulb, MessageCircle, Send, CornerDownRight, ExternalLink, X, LogIn } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import Navbar from '../components/Navbar';
import Footer from '../components/Footer';
import { useAuth } from '../auth/AuthContext';
import { blogApi, type BlogPost, type BlogComment, type ReactionType } from '../lib/blogApi';

const REACTION_CONFIG: { type: ReactionType; icon: any; label: string; color: string; bg: string }[] = [
  { type: 'support', icon: Heart, label: 'Support', color: 'text-emerald-600', bg: 'bg-emerald-100' },
  { type: 'inspire', icon: Sparkles, label: 'Inspire', color: 'text-amber-500', bg: 'bg-amber-100' },
  { type: 'care', icon: HandHeart, label: 'Care', color: 'text-rose-500', bg: 'bg-rose-100' },
  { type: 'celebrate', icon: PartyPopper, label: 'Celebrate', color: 'text-purple-500', bg: 'bg-purple-100' },
  { type: 'insightful', icon: Lightbulb, label: 'Insightful', color: 'text-blue-500', bg: 'bg-blue-100' },
];
...
function PostCard({ post, user, token, onNeedLogin }: { post: BlogPost; user: any; token: string | null; onNeedLogin: () => void }) {
  ...
  const renderComment = (c: BlogComment & { replies: BlogComment[] }, depth = 0) => (
    <div key={c.id} className={`${depth > 0 ? 'ml-8 border-l-2 border-slate-100 pl-4' : ''}`}>
  ...
  return (
    ...
    >
      ...
      <div className="px-6 pt-5 pb-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border ${getRoleBadgeColor(post.author_role)}`}>
              {post.author_role}
            </span>
          </div>
          <p className="text-[11px] text-slate-400 font-bold">{formatTimeAgo(post.created_at)}</p>
        </div>
      </div>
      ...
      <div className="px-6 pb-3">
        ...
        {post.link_url && (
          <a href={post.link_url} target="_blank" rel="noopener noreferrer"
            className="mt-3 flex items-center gap-2 text-emerald-600 hover:text-emerald-700 text-sm font-bold bg-emerald-50 px-4 py-2.5 rounded-xl transition-colors border border-emerald-100">
            <ExternalLink size={14} /> {post.link_url.length > 50 ? post.link_url.slice(0, 50) + '…' : post.link_url}
          </a>
        )}
      </div>
      ...
      {post.media_urls.length > 0 && (
        <div className="relative">
          {post.media_types[mediaIndex] === 'video' ? (
            <video src={post.media_urls[mediaIndex]} controls className="w-full max-h-[500px] object-contain bg-black" />
          ) : (
            <img src={post.media_urls[mediaIndex]} alt="" className="w-full max-h-[500px] object-cover" />
          )}
      ...
      {(totalReactions > 0 || post.commentCount > 0) && (
        <div className="px-6 py-2.5 flex items-center justify-between text-xs text-slate-400 border-b border-slate-50">
      ...
      {/* Action Bar */}
      <div className="px-4 py-1.5 flex items-center border-b border-slate-50 relative">
        <div
        ...
        <button onClick={toggleComments}
          className="flex-1 flex items-center gap-2 px-4 py-2.5 rounded-xl text-slate-500 hover:bg-slate-50 font-bold text-sm transition-all justify-center">
              ...
              {user && token ? (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  ...
                  <div className="flex gap-3 items-end">
                    ...
                    <div className="flex-1 bg-slate-50 rounded-2xl px-4 py-2 flex items-center gap-2 border border-slate-100 focus-within:border-emerald-300 transition-colors">
                      <input
                        value={commentText}
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 text-center">
                  <a href="/login" className="inline-flex items-center gap-2 text-emerald-600 font-bold text-sm hover:text-emerald-700 bg-emerald-50 px-5 py-2.5 rounded-xl transition-colors border border-emerald-100">
...
const BlogPage = () => {
  ...
  const fetchPosts = async (p = 1) => {
    setLoading(true);
    const res = await blogApi.getPosts(p, 10);
    if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* Hero */}
      <section className="relative pt-32 pb-16 overflow-hidden bg-emerald-900">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <h1 className="text-5xl md:text-6xl font-black text-white mb-4">News & Updates</h1>
