...
const ShiftingEvaluationModal = ({ isOpen, onClose, appointment, role = 'staff' }: ShiftingEvaluationModalProps) => {
  if (!appointment) return null;

  const requiredDocs = [
    { name: "2x2 Picture (with name tag)", status: "Uploaded" },
    { name: "All Downloadable Grades", status: "Uploaded" },
    { name: "Latest COR", status: "Uploaded" },
    { name: "College Entrance Test Result", status: "Uploaded" }
  ];
          ...
          >
            ...
            <div className="p-6 sm:p-8 space-y-6 sm:space-y-8 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {requiredDocs.map((doc, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg border border-emerald-100 shrink-0">
                        {doc.status}
                      </span>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 pt-2">
                ...
                <button className="group flex sm:flex-col items-center gap-3 p-4 sm:p-6 bg-rose-50 hover:bg-rose-600 rounded-[1.5rem] sm:rounded-[2rem] border border-rose-100 hover:border-rose-600 transition-all shadow-sm hover:shadow-rose-200">
