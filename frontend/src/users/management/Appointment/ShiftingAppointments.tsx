...
const ShiftingAppointments = ({ role = 'staff' }: { role?: 'staff' | 'director' | 'admin' }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
          {role === 'director' && appointments.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              <button className={`flex-1 sm:flex-initial justify-center px-4 py-2.5 ${theme.bg50} ${theme.hoverBg600} ${theme.text600} hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border ${theme.border200} ${theme.hoverBorder600} flex items-center gap-2 shadow-sm`}>
                <CheckCircle2 size={14} /> Accept All
              </button>
              <button className="flex-1 sm:flex-initial justify-center px-4 py-2.5 bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border border-rose-200 hover:border-rose-600 flex items-center gap-2 shadow-sm">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              ...
              <input type="text" placeholder="Search by student or course..." className="bg-white border border-slate-200 rounded-lg py-2.5 pl-10 pr-4 text-xs font-bold w-full focus:ring-2 focus:ring-rose-500 transition-all outline-none" />
            </div>
      ...
      {/* Config Summary Card */}
      <div className={`bg-white rounded-lg border border-slate-200 shadow-sm p-4 sm:p-5 transition-all duration-500 ${configLoading ? 'opacity-60 pointer-events-none' : ''}`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 ${status.color} rounded-xl flex items-center justify-center text-white shadow shrink-0`}>
          ...
          <div className="flex items-center gap-3 self-start md:self-auto">
            ...
            {role === 'director' && (
              <button
                ...
                className="px-4 sm:px-5 py-2 sm:py-2.5 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2 shadow-sm text-slate-700"
              >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className={theme.bg900}>
                <th className="px-4 sm:px-6 lg:px-8 py-3.5 sm:py-4 text-[10px] font-black uppercase tracking-widest text-white">Student Info</th>
                {role === 'director' ? (
                  ...
                  <th className="px-4 sm:px-6 lg:px-8 py-3.5 sm:py-4 text-[10px] font-black uppercase tracking-widest text-white">Current Course</th>
                )}
                <th className="px-4 sm:px-6 lg:px-8 py-3.5 sm:py-4 text-[10px] font-black uppercase tracking-widest text-white">Transition</th>
